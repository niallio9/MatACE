...
clim_type = '_sap_pm'; % this is where you choose what type of climatology to use: '_am', '_pm', '_sap', '_sap_am', '_sap_pm', or ''.

%STANDARD
nofolder = strcat('NO',clim_type);
no2folder = strcat('NO2',clim_type);
clono2folder = strcat('ClONO2',clim_type);
n2o5folder = strcat('N2O5',clim_type);
hno3folder = strcat('HNO3',clim_type);
hno4folder = strcat('HNO4',clim_type);
noyfolder = strcat('NOy',clim_type);
...
gasdir_clono2 = fullfile(climdir, clono2folder); % make the full path to the folder for NO
gasdir_n2o5 = fullfile(climdir,n2o5folder); % make the full path to the folder for NO
gasdir_hno3 = fullfile(climdir, hno3folder); % make the full path to the folder for NO
gasdir_hno4 = fullfile(climdir, hno4folder); % make the full path to the folder for NO
gasdir_noy = fullfile(climdir, noyfolder); % make the full path to the folder for NO
lgas = 6 + 1; % the plus 1 is for having to count N2O5 twice in the NOy summation
...
if ~isempty(climfile_lat_serial_no)
    ...
    for j = 1:length(yearandmonth) % go through each file NO file available and see if there is an NO2 file for the same year/month
        ...
        clono2_yearandmonth = sprintf('ACEFTS_CLIM_v3_lat_%s_%s.mat',clono2folder,yearandmonth{j});
        climfile_clono2 = fullfile(gasdir_clono2,'serial_month', clono2_yearandmonth); % the name of a climatology file
        n2o5_yearandmonth = sprintf('ACEFTS_CLIM_v3_lat_%s_%s.mat',n2o5folder,yearandmonth{j});
        climfile_n2o5 = fullfile(gasdir_n2o5,'serial_month', n2o5_yearandmonth); % the name of a climatology file
        hno3_yearandmonth = sprintf('ACEFTS_CLIM_v3_lat_%s_%s.mat',hno3folder,yearandmonth{j});
        climfile_hno3 = fullfile(gasdir_hno3,'serial_month', hno3_yearandmonth); % the name of a climatology file
        hno4_yearandmonth = sprintf('ACEFTS_CLIM_v3_lat_%s_%s.mat',hno4folder,yearandmonth{j});
        ...
        if exist(climfile_no2,'file') == 2 && exist(climfile_hno3,'file') == 2 && exist(climfile_clono2,'file') == 2 && exist(climfile_n2o5,'file') == 2 && exist(climfile_hno4,'file') ==2 % check if each file exists for that month
            ...
            climstruct_noy.beta_angle_mean = (climstruct_no.beta_angle_mean + climstruct_no2.beta_angle_mean + climstruct_hno3.beta_angle_mean + climstruct_clono2.beta_angle_mean + 2*climstruct_n2o5.beta_angle_mean + climstruct_hno4.beta_angle_mean)/lgas;
            climstruct_noy.doy_mean = (climstruct_no.doy_mean + climstruct_no2.doy_mean + climstruct_hno3.doy_mean + climstruct_clono2.doy_mean + 2*climstruct_n2o5.doy_mean + climstruct_hno4.doy_mean)/lgas;
            climstruct_noy.start_date = datestr(datenum(climstruct_no.start_date) + datenum(climstruct_no2.start_date) + datenum(climstruct_hno3.start_date) + datenum(climstruct_clono2.start_date) + 2*datenum(climstruct_n2o5.start_date) + datenum(climstruct_hno4.start_date))/lgas;
            climstruct_noy.end_date = datestr(datenum(climstruct_no.end_date) + datenum(climstruct_no2.end_date) + datenum(climstruct_hno3.end_date) + datenum(climstruct_clono2.end_date) + 2*datenum(climstruct_n2o5.end_date) + datenum(climstruct_hno4.end_date))/lgas;
            climstruct_noy.gas = noyfolder;
            climstruct_noy.lon_tangent_mean = (climstruct_no.lon_tangent_mean + climstruct_no2.lon_tangent_mean + climstruct_hno3.lon_tangent_mean + climstruct_clono2.lon_tangent_mean + 2*climstruct_n2o5.lon_tangent_mean + climstruct_hno4.lon_tangent_mean)/lgas;
            climstruct_noy.lat_tangent_mean = (climstruct_no.lat_tangent_mean + climstruct_no2.lat_tangent_mean + climstruct_hno3.lat_tangent_mean + climstruct_clono2.lat_tangent_mean + 2*climstruct_n2o5.lat_tangent_mean + climstruct_hno4.lat_tangent_mean)/lgas;
            climstruct_noy.altitude_km_mean = (climstruct_no.altitude_km_mean + climstruct_no2.altitude_km_mean + climstruct_hno3.altitude_km_mean + climstruct_clono2.altitude_km_mean + 2*climstruct_n2o5.altitude_km_mean + climstruct_hno4.altitude_km_mean)/lgas;
            climstruct_noy.pressure_hPa = climstruct_no.pressure_hPa; %this should be the same for all gases
            climstruct_noy.vmr_zonal = climstruct_no.vmr_zonal + climstruct_no2.vmr_zonal + climstruct_hno3.vmr_zonal + climstruct_clono2.vmr_zonal + 2*climstruct_n2o5.vmr_zonal + climstruct_hno4.vmr_zonal; % straight summation
            climstruct_noy.vmr_zonal_var = climstruct_no.vmr_zonal_var + climstruct_no2.vmr_zonal_var + climstruct_hno3.vmr_zonal_var + climstruct_clono2.vmr_zonal_var + 2*climstruct_n2o5.vmr_zonal_var + climstruct_hno4.vmr_zonal_var; % errors added in quadrature is the same as adding variances
            climstruct_noy.vmr_zonal_error = sqrt(climstruct_no.vmr_zonal_error.^2 + climstruct_no2.vmr_zonal_error.^2 + climstruct_hno3.vmr_zonal_error.^2 + climstruct_clono2.vmr_zonal_error.^2 + 2*(climstruct_n2o5.vmr_zonal_error.^2) + climstruct_hno4.vmr_zonal_error.^2); % add in quadrature
            climstruct_noy.vmr_zonal_standard_error = sqrt(climstruct_no.vmr_zonal_standard_error.^2 + climstruct_no2.vmr_zonal_error.^2 + climstruct_hno3.vmr_zonal_error.^2 + climstruct_clono2.vmr_zonal_error.^2 + 2*(climstruct_n2o5.vmr_zonal_error.^2) + climstruct_hno4.vmr_zonal_error.^2);
            ...
            climstruct_noy.lon_mean = (climstruct_no.lon_mean + climstruct_no2.lon_mean + climstruct_hno3.lon_mean + climstruct_clono2.lon_mean + 2*climstruct_n2o5.lon_mean + climstruct_hno4.lon_mean)/lgas;
            ...
            climstruct_noy.lst_median = (climstruct_no.lst_median + climstruct_no2.lst_median + climstruct_hno3.lst_median + climstruct_clono2.lst_median + 2*climstruct_n2o5.lst_median + climstruct_hno4.lst_median)/lgas;
            climstruct_noy.lst_mean = (climstruct_no.lst_mean + climstruct_no2.lst_mean + climstruct_hno3.lst_mean + climstruct_clono2.lst_mean + 2*climstruct_n2o5.lst_mean + climstruct_hno4.lst_mean)/lgas;
            climstruct_noy.lst_max = (climstruct_no.lst_max + climstruct_no2.lst_max + climstruct_hno3.lst_max + climstruct_clono2.lst_max + 2*climstruct_n2o5.lst_max + climstruct_hno4.lst_max)/lgas;
            climstruct_noy.lst_min = (climstruct_no.lst_min + climstruct_no2.lst_min + climstruct_hno3.lst_min + climstruct_clono2.lst_min + 2*climstruct_n2o5.lst_min + climstruct_hno4.lst_min)/lgas;
            climstruct_noy.lst_var = climstruct_no.lst_var + climstruct_no2.lst_var + climstruct_hno3.lst_var + climstruct_clono2.lst_var + 2*climstruct_n2o5.lst_var + climstruct_hno4.lst_var;
            climstruct_noy.climatology_type = climstruct_no.climatology_type; % should be the same for all gases
            climstruct_noy.time = climstruct_no.time; % should be the same for all gases
            % save the file
            climstruct = climstruct_noy;
            savedest = fullfile(gasdir_noy,'serial_month',sprintf('ACEFTS_CLIM_v3_lat_%s_%s.mat', noyfolder, yearandmonth{j}));
            fprintf('\nSaving %s %s climatology to %s\n', noyfolder, yearandmonth{j}, savedest);
