...
% *INPUT*    
%           cmamstruct: STRUCTURE - the name of the .nc file that holds the
%           CMAM data. Can be created with 'read_cmam_ncdata.m'.
%
% *OUTPUT*
%           tanstruct: STRUCTURE - with fields that correspond to the fields
...
if isfield(cmam,'occultation')
   fprintf('The CMAM structure already contains occultation information.\nNothing was done to the format.\n\n')
   tanstruct = cmam;
   return
end
% disp('line 25')
...
out.occultation = nan(1,lt*llon*llat);
out.sr1ss0 = nan(1,lt*llon*llat);
out.beta_angle = nan(1,lt*llon*llat);
...
out.quality_flags = nan(lalt,lt*llon*llat);
out.pressure_hPa = repmat(cmam.pressure_hPa, 1, lt*llon*llat); % need the number or pressure profiles to match the number of occultations
out.lat = repmat(latout,lalt,1); % want these for now so that it mimics the inclusion of GLC information in the ACE data structures.
out.lon = repmat(lonout,lalt,1);
