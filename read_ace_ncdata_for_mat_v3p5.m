function [ ] = read_ace_ncdata_for_mat_v3p5( varargin )
...
ncdirectory = 'C:\Users\ryann\jaho\acefts_v3_5_all_flag\';
...
matdirectory = 'C:\Users\ryann\jaho\matdata_v3p5_nr\';

%%STANDARD
filein_pre = 'ACEFTS_L2_v3p5_';
filein_post = '.nc';
savename_pre = 'ACE_v3p5_';
%% read the pressure file and get the data
pfile = strcat(ncdirectory,filein_pre,'P',filein_post);
[pstruct] = read_ace_ncdata_v3p5(pfile);
pace = pstruct.vmr * 1013.25;
clear pstruct
        ...
        if ~strcmp(gasin,'all') %%% a case of reading one .nc file
            ...
            if(~isempty(temp_dir)) % if the file exists
                if strcmp(gasin, 'GLC')
                    ...
                    [tanstruct, gasouti] = read_ace_ncdata_v3p5(filein); %#ok<ASGLU> % read the data and create a matlab structure
                    tanstruct.pressure_hPa = pace; % add the pressure information
                    savedest = strcat(matdirectory,savename_pre,gasouti);
                    fprintf('saving %s data to %s\n', gasouti, savedest);
