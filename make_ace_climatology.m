function [ testout ] = make_ace_climatology( tanstruct)
...
outdir = strcat(home_mac,'matclim/');
...
for i = 1:12
    ...
    if  nansum(gas_monthi_latbin.date_mjd_mean) ~= 0
        climstruct = gas_monthi_latbin;
        savename_post = sprintf('_%02d',i);
        savedest = strcat(outdir,savename_pre,gas.gas,savename_post);
