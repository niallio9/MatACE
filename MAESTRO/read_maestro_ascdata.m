function [ maestro_out ] = read_maestro_ascdata( path_to_file)
...
for n = 7
    tline = fgetl(fid1); % there are 10 header lines before the data
    if tline == -1
...
i = 0;
for n = 11:321 %the number of lines that contain the data table.
    i = i+1;
    tline = fgetl(fid1);
    C = textscan(tline, '%s');
    C = C{1,1};
