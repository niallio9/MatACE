function [ climstruct ] = make_ace_climatology_with_filter( tanstruct )
%A function to create zonally averaged climatologies of ACE measurements.
%The function assumes that the ACE geoloction (GLC) data or the Derived
%meteorological product(DMP)data has been added to the gas data. This can
%be done using 'merge_ace_glc.m' or 'merge_ace_dmp.m'. A MAD filter is
%applied to the data in each latitude bin.
...
gas_latbin  = bin_ace_by_lat_with_filter( gas, lat_bounds );
