function [ gas_in ] = write_ace_climatology_to_netcdf( varargin )
 %A function to create an ACE climatology netCDF file from the information
 %contained in the ACE climstruct .mat file.
%
% *INPUT*
%
%           gas: STRING - the name of the gas for which you want to write a
    ...
    for i = 1:length(gasfolder) % loop through the gases
        ...
        if ~isempty(climfile_lat_serial)
            ...
            for j = 1:length(data_years_unique) % make a netcdf file for each year of data
                ...
                outdir = fullfile(ncdir, gasfolder{i});
                if exist(outdir,'dir') ~= 7
                    mkdir(outdir);
                end
                ncfilename_j = fullfile(outdir, ncfilename_j);
                make_ace_empty_netcdf_serialmonth(ncfilename_j,gasfolder{i}); % make the empty .nc file (no data)
                fprintf('writing the %i data...\n', data_years_unique(j))
                ncwrite(ncfilename_j, gasfolder{i}, vmr_out);
                ncwrite(ncfilename_j, strcat(gasfolder{i},'_STD'), vmr_std_out);
                ncwrite(ncfilename_j, strcat(gasfolder{i},'_NR'), vmr_obs_out);
