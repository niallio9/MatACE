function [ vmrzon ] = plot_ace_climatology_bygas_serialmonth_timeseries( gasname_in, years_in, lat_minmax, do_plot )
...
vmrzon = squeeze(nanmean(vmrzon(:,ilatmin:ilatmax,:), 2 )) * 1e6;
vmrzon_error = squeeze(nanmean(vmrzon_error(:,ilatmin:ilatmax,:), 2 )) * 1e6;
...
%     figi = 19;
    ...
    for i = 1:nalt
        ...
        %             ylim([0,4.0])
        ytext = max(vmrzon(ipplot(i),:))*1.5; % the position for the text in the plots, in ppbv
