    ...
    if ~isempty(Itime)
        ...
        if ~isempty(mls_h.occultation)
            ...
            if ~isempty(Id)
                ...
                warning off
                mls_h_int_d = reduce_tanstruct_data_by_index(mls_h_int, Id); % this will give an empty matrix if there are no points that fit the criteria.
                warning on
                ...
                if ~isempty(Iv)
                    warning off
                    mls_h_int_d_v = reduce_tanstruct_data_by_index(mls_h_int_d, Iv);
                    warning on
