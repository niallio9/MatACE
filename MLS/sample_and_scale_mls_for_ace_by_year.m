function [ ] = sample_and_scale_mls_for_ace_by_year(tanstruct_in, gas_name, years_in, save_appendix)
...
pratdir = 'C:\Users\ryann\ACE\matdata\pratmo\default_runs\';
...
mlsfile_pre = sprintf('MLS_v4p2_%s_dmp_', gas_name);
mlsfile_post = '.mat';
pratfile_pre = sprintf('ACE_v3p6_pratmo_%s_all_LST_', gas_name);
pratfile_post = '.mat';
...
if ~isdir(pratdir)
    fprintf('\nIt doesn''t look like ''%s'' exists...\n', pratdir)
    error('The PRATMO directory couldn''t be found')
end
...
for i = 1:lyears
    ...
    %% read the pratmo data
    pratfile = fullfile(pratdir, strcat(pratfile_pre, num2str(yearsin(i)), pratfile_post));
    fprintf('\nloading %s...\n', pratfile)
    if exist(pratfile,'file') ~= 2
        error('cannot find %s', pratfile)
    end
    ...
    %% do sampling with the yearly data, without filtering
    disp('applying sampling function to the data...')
%     saveappendix_frac = strcat('frac50_',saveappendix);
    sample_and_scale_mls_for_ace(mlsstruct_i, acestruct_i, gas_name, pratstruct, saveappendix, 4e-9);
...
%     sample_and_scale_mls_for_ace(mlsstruct_i_am, acestruct_i_am, gas_name, pratstruct, saveappendix_am, mls_minmax);
%     disp('applying sampling function to the pm data...')
%     saveappendix_pm = strcat('pm_',saveappendix);
%     sample_and_scale_mls_for_ace(mlsstruct_i_pm, acestruct_i_pm, gas_name, pratstruct, saveappendix_pm, mls_minmax);
