...
switch gas
    case 'ClO'
        ...
        % Aura MLS website
        load('MLS_ClO_bias_correction.mat'); % loads as 'bias_correction'. This must be downloaded from the MLS website
...
if apply_pressure_lim == 1 && ~strcmp(gas,'ScaledPV')
...
switch gas
    case 'ClO'
        if ~isempty(mls_out.vmr) % if there the output structure is not empty
