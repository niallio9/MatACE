function [ tanstruct_before, tanstruct_after ] = subset_ace_by_lst_tangent( tanstruct_in, lst_break )
...
lst_ace = get_ace_lst_tangent(gas); % get a vector of the LSTs of the occultations
ilst_before = find( lst_ace <= lstbreak ); % get the indices of the occultations that are before the input LST
ilst_after = find( lst_ace > lstbreak ); % get the indices of the occultations that are before the input LST

%Subset the data
gas_before = reduce_tanstruct_by_rowindex(gas, ilst_before);
gas_after = reduce_tanstruct_by_rowindex(gas, ilst_after);
